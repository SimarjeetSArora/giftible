...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT p.*, c.name as `service` from `serviceoff_list` p inner join service_list c on p.service_id = c.id where p.id = '{$_GET['id']}' and p.delete_flag = 0 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	<center><img src="<?= validate_image(isset($image_path) ? $image_path : "") ?>" alt="Service Image" class="img-thubmnail p-0 bg-gradient-gray" id="serv-img-view"></center>
	<dl>
        <dt class="text-muted">Service</dt>
        <dd class="pl-3"><?= isset($name) ? $name : "" ?></dd>
        <dt class="text-muted">Service Category</dt>
