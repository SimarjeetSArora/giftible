...
Class Master extends DBConnection {
	...
	function save_service(){
		...
		$check = $this->conn->query("SELECT * FROM `service_list` where `name` = '{$name}' and spable_id = '{$spable_id}' and delete_flag = 0 ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
	...
	function save_serviceoff(){
		...
		$check = $this->conn->query("SELECT * FROM `serviceoff_list` where spable_id = '{$spable_id}' and `service_id` = '{$service_id}' and delete_flag = 0 ".(!empty($id) ? " and id != '{$id}'" : ""))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Service Name Already exists.';
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `serviceoff_list` set {$data} ";
			}else{
				$sql = "UPDATE `serviceoff_list` set {$data} where id = '{$id}' ";
			}
			$save = $this->conn->query($sql);
			if($save){
				$pid = empty($id) ? $this->conn->insert_id : $id;
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					if(!is_dir(base_app."uploads/serviceoff"))
					mkdir(base_app."uploads/serviceoff");
					$fname = 'uploads/serviceoff/'.($pid).'.png';
					$dir_path =base_app. $fname;
					$upload = $_FILES['img']['tmp_name'];
					$type = mime_content_type($upload);
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if(isset($uploaded_img) && $uploaded_img == true){
									$qry = $this->conn->query("UPDATE `serviceoff_list` set image_path = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$pid' ");
								}
						}else{
						$resp['msg']=" But Image failed to upload due to unkown reason.";
						}
	...
	function delete_serviceoff(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `serviceoff_list` set `delete_flag` = 1 where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Service successfully deleted.");
		}else{
