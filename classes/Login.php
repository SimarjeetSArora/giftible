...
class Login extends DBConnection {
	...
	public function login_spable(){
		extract($_POST);
		$password = md5($password);
		$stmt = $this->conn->prepare("SELECT * from spable_list where username = ? and `password` =? and delete_flag = ?  ");
		$delete_flag = 0;
		$stmt->bind_param("ssi",$username,$password,$delete_flag);
		$stmt->execute();
		$result = $stmt->get_result();
...
switch ($action) {
	...
	case 'logout_vendor':
		...
		break;
	case 'login_spable':
		echo $auth->login_spable();
		break;
	case 'logout_spable':
		echo $auth->logout_spable();
		break;
	default:
		echo $auth->index();
		break;
