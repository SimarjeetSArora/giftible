...
Class Users extends DBConnection {
	...
	public function save_spable(){
		...
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `spable_list` where code = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
					$code = sprintf("%'.05d",ceil($code) + 1);
				}else{
					break;
		...
		extract($_POST);
		if(isset($oldpassword) && !empty($id)){
			$current_pass = $this->conn->query("SELECT * FROM `spable_list` where id = '{$id}'")->fetch_array()['password'];
			if(md5($oldpassword) != $current_pass){
				$resp['status'] = 'failed';
				$resp['msg'] = ' Incorrect Current Password';
				return json_encode($resp);
		...
		$check  = $this->conn->query("SELECT * FROM `spable_list` where username = '{$username}' and delete_flag = 0 ".(!empty($id) ? " and id !='{$id}'" : ''))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Username already exists";
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `spable_list` set {$data}";
			}else{
				$sql = "UPDATE `spable_list` set {$data} where id = '{$id}'";
			}
			$save = $this->conn->query($sql);
			if($save){
				$resp['status'] = "success";
				$vid = empty($id) ? $this->conn->insert_id : $id;
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'spable/register.php') > -1){
						$resp['msg'] = " Your account has been registered successfully.";
					}else{
						$resp['msg'] = " Specially Abled Account has been registered successfully.";
					}
				}else{
					if($this->settings->userdata('login_type') == 4){
						$resp['msg'] = " Your account details has been updated successfully.";
					}else{
						$resp['msg'] = " Specially Abled Account Details has been updated successfully.";
					}	
				}

				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					if(!is_dir(base_app."uploads/spable"))
					mkdir(base_app."uploads/spable");
					$fname = 'uploads/spable/'.($vid).'.png';
					$dir_path =base_app. $fname;
					$upload = $_FILES['img']['tmp_name'];
					$type = mime_content_type($upload);
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `spable_list` set avatar = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
									if($this->settings->userdata('id') == $id && $this->settings->userdata('login_type') == 4)
										$this->settings->set_userdata('avatar',$fname."?v=".(time()));
								}
						}else{
						$resp['msg'].=" But Image failed to upload due to unkown reason.";
				...
				if(isset($_FILES['disability_id']) && $_FILES['disability_id']['tmp_name'] != ''){
					if(!is_dir(base_app."uploads/spable/id"))
					mkdir(base_app."uploads/spable/id");
					$fname = 'uploads/spable/id/id_'.($vid).'.png';
					$dir_path =base_app. $fname;
					$upload = $_FILES['disability_id']['tmp_name'];
					$type = mime_content_type($upload);
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						$resp['msg'].=" But Image failed to upload due to invalid file type.";
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `spable_list` set disability_id = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
									if($this->settings->userdata('id') == $id && $this->settings->userdata('login_type') == 4)
										$this->settings->set_userdata('disability_id',$fname."?v=".(time()));
								}
						}else{
						$resp['msg'].=" But Image failed to upload due to unkown reason.";
						}
	...
	public function delete_spable(){
		extract($_POST);
		$qry = $this->conn->query("UPDATE spable_list set delete_flag = 1 where id = $id");
		if($qry){
			$this->settings->set_flashdata('success',' Specially Abled Details successfully deleted.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'An error occured while deleting the data.';
...
switch ($action) {
	...
	break;
	case 'save_spable':
		echo $users->save_spable();
	break;
	case 'delete_spable':
		echo $users->delete_spable();
	break;
	case 'save_client':
